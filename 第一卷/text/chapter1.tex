...
\footnote{原书沿用“整序变量”这一历史术语. 指定整序变量与指定数列在本质上是同一件事: 都是按照某种确定规则, 使每个自然数 $n$ 对应一个确定的实数 $x_n$. 现代数学教材通常使用“数列”, 本书重排本亦统一采用这一名称.}
...
在定义圆周长时, 还会考察圆内接正多边形的周长: 从正六边形开始, 每次将边数加倍, 可得到数列
$$
P_6,\ P_{12},\ P_{24},\ P_{48},\ \cdots.
$$
若圆的半径为 $R$, 则内接正 $m$ 边形的周长为
$$
P_m=2mR\sin\frac{\pi}{m}.
$$
...
“无穷小”这一历史名称容易引起误解. 除恒等于 $0$ 的平凡情形外, 无穷小数列的某个单独项不能被笼统地称为“很小的量”; 这里描述的是数列的变化过程, 即它的绝对值最终能小于任意预先给定的正数.
...
\myrefparagraph{1)}\label{ex:infinitesimal_basic_sequences}
    ...
    \footnote{$\floor{p}$ 表示不超过 $p$ 的最大整数, 或简称数 $p$ 的整数部分.},
    就能证明了这三个数列都是无穷小.
...
\myrefparagraph{4)}\label{ex:rational_sequence_one_third}
    ...
    $$
    则当 $n>N_\epsilon$ 时既有 $n>2$, 又有 $1/n<\epsilon$, 从而 $\abs{x_n-\dfrac{1}{3}}<\epsilon$, 即 $x_n\to\dfrac{1}{3}$.
...
$$
就证明了我们的命题. 这里取最大值是为了保证当 $0<E<1$ 时, $N_E$ 仍是正整数.
因此, 当 $\abs{Q}>1$ 时, $Q^n$ 是无穷大量.
同样, 当 $Q>1$ 时有 $Q^n\to+\infty$;
当 $Q<-1$ 时, $Q^n$ 的绝对值趋于 $+\infty$, 但它本身没有带符号的无穷极限.

回到最初的三个例子, $\seqset{x}$ 始终带正号, $\seqset{y}$ 始终带负号, $\seqset{z}$ 则正负交替出现.
前两种情形可以进一步细分:
...
关于这些“广义的数” $\pm\infty$, 我们在 \emph{\ref{subsec:11}} 已讨论过它们.
它们必须结合上述定义使用, 不能不加条件地当作普通实数进行四则运算.
按照习惯, 语境明确时常用 $\infty$ 代替 $+\infty$.

引入无穷极限并不会破坏命题 \ref{prop:limit_uniqueness} 中建立的极限唯一性.
事实上, 有有限极限的数列必定有界, 因而不可能同时趋于无穷.
