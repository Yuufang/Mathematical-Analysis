...
但在数学上我们并不考察量所蕴含的物理意义, 仅仅关心表示这量所表示的数字.
...
本章先考察最简单而且最重要的一类情形: 变量的数值由自然数编号, 并按编号的次序逐一出现.
这就需要我们建立\emph{数列}\index{shulie@数列}的概念.
...
这样得到的对象就是数列, 并将序列 \ref{eq:real_seq} 简记成 $\{x_n\}$,
...
$$
    \underset{1}{1.4}, \underset{2}{1.41},\underset{3}{1.414},\underset{3}{1.4142}, \cdots
$$
同样得到一个数列.
...
{\kaishu
    ...
    \begin{equation} \label{limit_definition1}
        \abs{x_n-a}<\epsilon,
    \end{equation}
...
一般来说, 数 $\epsilon$ 减小时, 与它对应的序数 $N = N_{\epsilon}$, 将会增大: 因为要使 $x_n$ 的值与 $a$ 的接近程度愈大, 我们在数列 $\seqset{x}$ 内所要考察的数值序号便必须愈大.

数列 $\seqset{x}$ 的一切数值都等于常量 $a$ 的这情形是例外.
显然, 这时 $a = \displaystyle \limto{n} x_n$,
因为这时对于任何 $\epsilon > 0$ , 不等式 \eqref{limit_definition1} 能同时对于 $x_n$ 的一切值都成立.

在 \emph{\ref{subsec:17}} 中我们知道, 不等式 \eqref{limit_definition1} 相当于下列不等式
$$
...
则不等式 \eqref{limit_definition1} 成为
$$
\abs{x_n - 0} = \abs{x_n} < \epsilon \quad (n > N_\epsilon).
$$
这样, 上面无穷小的定义可以不用术语“极限”而更详细地叙述成:
{\kaishu
若数列 $\seqset{x}$ 的绝对值, 自某项起, 成为而且永远保持小于预先指定的任意小数 $\epsilon > 0$, 则它称为无穷小.
}
...
\myrefparagraph{1)}\label{ex:infinitesimal_basic_sequences}
    ...
    $$
    \begin{array}{ccccc}
    \phantom{-}1, & \phantom{-}\dfrac{1}{2}, & \phantom{-}\dfrac{1}{3} & \phantom{-}\dfrac{1}{4}, & \cdots \\
    ...
    令 $N_\epsilon = \bbrac{\dfrac{1}{\epsilon}}$
    \footnote{$\bbrac{p}$ 表示不超过 $p$ 的最大整数, 或简称数 $p$ 的整数部分.},
    就能证明了这三个数列都是无穷小.
    尽管它们在 $0$ 的同一侧、另一侧或两侧交替出现, 但这都不影响最终极限为 $0$.
...
\myrefparagraph{2)}\label{ex:oscillatory_zero_limit}
    ...
    \qquad
    \text{当 } \ n>\bbrac{\frac{3}{\epsilon}}\text{ 时}.
    $$
    因而 $x_n\to0$.
...
\myrefparagraph{3)}\label{ex:zero_odd_terms_limit}
    ...
    \qquad\text{当 } \ n>\bbrac{\frac{2}{\epsilon}}\text{ 时},
    $$
    所以 $x_n\to0$.
    数列可以在无穷多个位置恰好取到极限值, 这同样不妨碍极限的存在.

    \ref{ex:infinitesimal_basic_sequences}-\ref{ex:zero_odd_terms_limit}
...
\myrefparagraph{4)}\label{ex:rational_sequence_one_third}
    ...
    $$
    故当 $n>N_\epsilon = \bbrac{\dfrac{1}{\epsilon}}$ 时有 $\abs{x_n-\dfrac{1}{3}}<\epsilon$, 从而 $x_n\to\dfrac{1}{3}$.
...
\myrefparagraph{5)}\label{ex:nth_root_constant_supplement}
    ...
    $$
    \abs{x_n-1} = \sqrt[n]{a} - 1 < \frac{a-1}{n} < \epsilon, \quad \text{当} \ n > N_\epsilon = \bbrac{\dfrac{a-1}{\epsilon}}.
    $$
    亦可以直接运用对数
    $$
    \abs{x_n-1} = \sqrt[n]{a} - 1 < \epsilon \Rightarrow \frac{1}{n} < \log_a(1+\epsilon), \quad \text{当} \ n > N_\epsilon = \bbrac{\dfrac{1}{\log_a(1+\epsilon)}}.
    $$
    ...
    而依第二种方法得
    $
    N_{0.01} = \bbrac{\dfrac{1}{0.004 \ 32 \ \cdots}} = 231.
    $
    注意到
    $
    ...
    只需保证不等式 (\ref{limit_definition1}) 能成立就好了,
    至于从哪一项开始, 位置远些的活近些的, 可以不必管它.
...
\myrefparagraph{9)}\label{ex:series_partial_sums}
    ...
    $$
    A_1 = a_1, \ A_2 = a_1+a_2,\ \cdots,\ A_n=a_1+a_2+\cdots+a_n.
    $$
    若 $A_n\to A$, 就把 $A$ 定义为
    $$
...
设数列 $\seqset{x}$ 有极限 $a$, 那么应当存在不等式 (\ref{eq:limit_inequality}):
$$
a - \epsilon < x_n < a + \epsilon,
$$
也就是说我们的数列在某项之后与极限值应当具有一样的大小关系.
换句话来说, 若有数 $p<a$（或 $q>a$）,
...
它是命题 \emph{\ref{prop:eventual_order_from_limit}} 在 $p=0$（或 $q=0$）时的直接推论.
...
\abs{x_n}>r>0.
$$}
事实上, 当 $a>0$ 时, 可以任取 $0<r<a$, 再应用命题 \emph{\ref{prop:eventual_order_from_limit}}; 当 $a<0$ 时, 可以任取 $a<-r<0$. 因此, 一个趋于非零极限的数列, 从某一项开始总与 $0$ 保持正的距离.
...
$$}
取 $M'>\abs{a}$. 由命题 \emph{\ref{prop:eventual_order_from_limit}}, 存在 $N$, 使当 $n>N$ 时,
$$
...
命题 \emph{\ref{prop:convergent_sequence_bounded}} 的逆命题不成立: 有界数列未必收敛. 例如
$$
x_n=(-1)^{n+1}
$$
...
最后可以根据命题 \emph{\ref{prop:eventual_order_from_limit}} 证明极限的\emph{唯一性}.
...
a<r<b.
$$
由命题 \emph{\ref{prop:eventual_order_from_limit}}, 存在 $N'$, 使当 $n>N'$ 时有 $x_n<r$; 又存在 $N''$, 使当 $n>N''$ 时有 $x_n>r$. 于是当
$$
n>\max\bbbrac{N',N''}
$$
...
进一步地可以发现,
$\seqset{x}$ 在变化过程中始终带正号, $\seqset{y}$ 始终带负号, $\seqset{z}$ 则是正负交替出现.

我们可以将前面两种情况细分:
{\kaishu 若 $\seqset{x}$ 成为无穷大, 并且(至少在充分大的 $n$ 时)保持着一定的符号(+或−),
这时按照符号的正或负, 我们说 $\seqset{x}$ 有极限 $+\infty$ 或 $-\infty$, 并写成:
$$
\limto{n} x_n = +\infty, \ x_n \to +\infty \quad \text{或} \quad \limto{n} x_n = -\infty, \ x_n \to -\infty.
$$}
在这些情形时, 可以分别用不等式
$$
x_n > E \quad \text{或} \quad x_n < -E
$$
来代替不等式 $\abs{x_n} > E$, 以作为每种特殊无穷大量的定义式.
一般的无穷大量则可以统一写成
$$
\abs{x_n}\to+\infty.
$$
...
至于 $\{z_n\}$, 对于它我们既不能说它趋向 $+\infty$, 也不能说它趋向 $-\infty$,
仅能说极限不存在或者\emph{发散}.

关于这些“广义的数” $\pm\infty$, 我们在 \emph{\ref{subsec:11}} 已讨论过它.
...
也就是
$$
n>\frac{\ln E}{\ln\abs{Q}}.
$$
因此取数 $N_E = \bbrac{\dfrac{\ln E}{\ln\abs{Q}}}$ 就证明了我们的命题.
...
{\kaishu
    若 $\seqset{x}$ 是无穷大, 则它的倒数 $\alpha_n = \dfrac{1}{x_n}$ 将成无穷小.
}
\begin{proof}
    取任意数 $\epsilon > 0$. 因 $x_n \to \infty$, 故对于数 $E = \dfrac{1}{\epsilon}$ 可以求得序号 $N$, 使
$$
\abs{x_n} > \frac{1}{\epsilon}, \quad \text{仅需} \ n > N.
$$
于是对于这种 $n$, 显然将有
$$
...
{\kaishu
    若变量 $\alpha_n$ (不等于零) 是无穷小, 则其倒数 $x_n = \dfrac{1}{\alpha_n}$ 将成无穷大.
}
