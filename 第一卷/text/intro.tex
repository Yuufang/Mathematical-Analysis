...
对于每一对数 $a$ 及 $b$, 存在着一个唯一的数,被称为 $a$ 与 $b$ 的\emph{和}\index{youlishudehe@有理数的和} (记成 $a + b$).
这概念具有下列的性质:
{\kaishu
\begin{itemize}
...
{\kaishu
\begin{itemize}
    \myrefitem{III $1^\circ$}\label{ax:rational_mul_1} $ab = ba$ (乘法的交换律);
    \myrefitem{III $2^\circ$}\label{ax:rational_mul_2} $\brac{ab}c = a\brac{bc}$ (乘法的结合律).
\end{itemize}
}
“$1$” 这个数比较特殊, 它具有下列特性:
{\kaishu
\begin{itemize}
    \myrefitem{III $3^\circ$}\label{ax:rational_mul_3} $a \cdot 1 = a$;
\end{itemize}
}
此外,
{\kaishu
\begin{itemize}
    \myrefitem{III $4^\circ$}\label{ax:rational_mul_4} 对于每一异于 $0$ 的数 $a$, 必有数 $\dfrac{1}{a}$ (其倒数), 使 $a \cdot \dfrac{1}{a} = 1$.
\end{itemize}
}
...
{\kaishu
\begin{itemize}
    \myrefitem{III $5^\circ$}\label{ax:rational_mul_5} $\brac{a + b}c = a \cdot c + b \cdot c$ (乘法关于和的分配律).
\end{itemize}
}
...
再应用性质 \ref{ax:rational_mul_5}, 可证
$$
b \cdot 0 = 0 \cdot b = 0.
$$
实际上, 因为 [\ref{ax:rational_add_3}]
$$
a + 0 = a, \quad \brac{a + 0} \cdot b = a \cdot b + 0 \cdot b = a \cdot b,
$$
可推得 $0 \cdot b = 0$, 再由 [\ref{ax:rational_mul_1}] 得 $b \cdot 0 = 0$.
...
{\kaishu 
\begin{itemize}
    \myrefitem{III $6^\circ$}\label{ax:mul_6} 由 $a > b$ 及 $c > 0$ 可推得 $a \cdot c > b \cdot c$.
\end{itemize}
}
...
\index{Archimedesgongli@Archimedes 公理}

我们用下列简单而重要的论证, 来结束我们的有理数基本性质一览表.
这一性质是不能由上述的诸性质里推得的.
{\kaishu
\begin{itemize}
    \myrefitem{IV $1^\circ$}\label{ax:archimedes} 不论 $c > 0$ 是怎样的数, 总有大于 $c$ 的自然数 $n$ 存在着.
\end{itemize}
}

实际上, Archimedes 曾说明了一个几何的命题:
{\kaishu
...
{\kaishu
\begin{itemize}
    \myrefitem{1$^\circ$}\label{ax:rational_cut_1} 任意有理数, 必在且仅在 $A$ 与 $A'$ 这两个集合之一中出现
    \footnote{“任意有理数仅在两集合之一中出现” 这一事实亦可由性质 \ref{ax:rational_cut_2} 推得.};
    \myrefitem{2$^\circ$}\label{ax:rational_cut_2} 集合 $A$ 内的任意元素 $a$, 必定小于集合 $A'$ 内的任意元素 $a'$.
\end{itemize}
}
...
故不等式 $10^n \leq \dfrac{1}{e}$ 或等价的不等式 $\dfrac{1}{10^n}\ge e$ 仅能对有限个 $n$ 的值满足;
对于其余一切 $n$ 的值, 将有 
$$
\dfrac{1}{10^n} < e.
$$
...
{\kaishu
\begin{itemize}
    \myrefitem{$1^\circ$}\label{ax:real_cut_1} 每一实数必落在集 $A, A'$ 中一个且仅一个之内;
    \myrefitem{$2^\circ$}\label{ax:real_cut_2} 集 $A$ 的每一数 $\alpha$ 小于集 $A'$ 的每一数 $\alpha'$.
\end{itemize}
}
...
设有一实数的无限集, 比如自然数集, 一切真分数构成的集合, 在 $0$ 与 $1$ 之间的一切实数集,
...
}例如, 真分数集关于数 $1$ 或任何大于 $1$ 的数上有界, 自然数序列非上有界.
...
例如,自然数序列关于数 $1$ 或任何 $< 1$ 的数下有界, 真分数集关于 $0$ 或 $< 0$ 的数下有界.
