...
只要他们成立, 其余的一切性质都能作为形式逻辑的结果而从之推出了.
...
{\kaishu
\begin{itemize}
    \myrefitem{I $1^\circ$}\label{ax:rational_order_1}
    每一对数 $a$ 与 $b$ 之间必有且仅有下列关系之一:
    $$
    a = b, \quad a > b, \quad a < b;
    $$
    \myrefitem{I $2^\circ$}\label{ax:rational_order_2}
    由 $a > b$ 及 $b > c$ 可推得 $a > c$ ($>$ 的传递性);
    \myrefitem{I $3^\circ$}\label{ax:rational_order_3}
    若 $a > b$, 则必能求得一数 $c$, 使
...
{\kaishu
\begin{itemize}
    \myrefitem{II $1^\circ$}\label{ax:rational_add_1}
    $a + b = b + a$ (加法的交换律);
    \myrefitem{II $2^\circ$}\label{ax:rational_add_2}
...
{\kaishu
\begin{itemize}
    \myrefitem{II $4^\circ$}\label{ax:rational_add_4}
...
依 Dedekind 基本定理[\emph{\ref{subsec:10}}], 必有产生分划的实数 $\beta$ 存在.
...
(在这时, 可以应用与 \emph{\ref{subsec:14}} 内有关二乘数之积的相似的论证.
...
这个问题在几何学上的回答是肯定的.
即依靠直线的连续性公理, 它赋予了作为点的集合的直线以类似于实数域连续性的性质 [\emph{\ref{subsec:10}}].
