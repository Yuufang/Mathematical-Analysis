...
也就是说, {\kaishu 并不存在这样的有理数 $\dfrac{p}{q}$(式中 $p$ 及 $q$ 为自然数), 其平方能等于 $2$}.
...
在几何学上, 若我们仅停留在有理数的范围内, 那么并非一切的线段都能有一个{\kaishu 长度}.
例如, 考察边长为单位长度的正方形.
依勾股定理, 其对角线长度的平方应等于 $2$, 上面我们已经证过, 这不可能为有理长度 $\dfrac{p}{q}$.

无论是在代数还是几何上, 我们的有理数都显得不太够用,
为了解决类似的情况, 我们将在本绪论中为有理数域添上一种新的数, 称为无理数, 用以扩大有理数域的范围.
...
它的性质是平凡的, 没有什么可研究的.
我们更关心的是“不等”的情况, 这被称作 {\kaishu 序}.

有理数域的序得自“大于”($>$)的概念, 与之有关的是第一组性质:
{\kaishu
\begin{itemize}
    \myrefitem{I $1^\circ$}\label{ax:rational_order_1} 每一对数 $a$ 与 $b$ 之间必有且仅有下列关系之一:
    $$a = b, \quad a > b, \quad a < b;$$
    \myrefitem{I $2^\circ$}\label{ax:rational_order_2} 由 $a > b$ 及 $b > c$ 可推得 $a > c$ ($>$ 的传递性);
    \myrefitem{I $3^\circ$}\label{ax:rational_order_3} 若 $a > b$, 则必能求得一数 $c$, 使
...
说 $a < b$,就是等价于说 $b > a$.
显而易见, \ref{ax:rational_order_2} 和 \ref{ax:rational_order_3} 所代表的性质 “小于” 同样满足.

\subsection{有理数的加法及减法} \label{subsec:3}

第二组性质是关于加法的, 即关于求两数之和的运算性质.
对于每一对数 $a$ 及 $b$, 存在着一个唯一的数,被称为 $a$ 及 $b$ 的\emph{和}\index{youlishudehe@有理数的和} (记成 $a + b$).
这概念具有下列的性质:
{\kaishu
\begin{itemize}
    \myrefitem{II $1^\circ$}\label{ax:rational_add_1} $a + b = b + a$ (加法的交换律);
    \myrefitem{II $2^\circ$}\label{ax:rational_add_2} $\brac{a + b} + c = a + \brac{b + c}$ (加法的结合律).
\end{itemize}
}
“$0$” 这个数比较特殊, 它具有下列特性:
{\kaishu
\begin{itemize}
    \myrefitem{II $3^\circ$}\label{ax:rational_add_3} $a + 0 = a$;
\end{itemize}
}
此外,
\begin{itemize}
    \myrefitem{II $4^\circ$}\label{ax:rational_add_4} 对每一数 $a$ 存在着与它相反的数 $-a$, 使 $a + \brac{-a} = 0$.
\end{itemize}

在这些性质的基础上, 首先可以解决加法的逆运算, 即\emph{减法}的问题.
通常称使 $c + b = a$
\footnote{依 \ref{ax:rational_add_1}, 定义差的等式可写成: $b + c = a$.} 
的数 $c$, 为数 $a$ 及 $b$ 的\emph{差}\index{youlishudecha@有理数的差}.
伴随而来的问题就是, 这样的数的是否存在? 如果存在, 是否唯一?
...
{\kaishu
\begin{itemize}
    \myrefitem{II $5^\circ$}\label{ax:rational_add_5} 由 $a > b$ 可推得 $a + c > b + c$.
\end{itemize}
}
