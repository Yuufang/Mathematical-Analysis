...
n > \dfrac{2a + 1}{2 - a^2}.
$$
根据阿基米德公理 [\ref{ax:archimedes}], 这恒是可能的.
...
我们应用 Dedekind 基本定理 [\emph{\ref{thm:dedekind_cut}}],
...
这样任意实数的乘积就都可以计算了.
如果希望实数运算继续满足有理数运算的一切基本性质, 那么正如第 \ref{subsec:4} 节所说明的那样, 上述关于 $0$ 和符号的约定是必需的.
...
现在把一切负有理数及 $0$, 以及所有能作为短于 $\Sigma$ 的可通约线段之长度的正有理数 $s$, 归入下组 $A$;
把所有能作为长于 $\Sigma$ 的可通约线段之长度的正有理数 $s'$, 归入上组 $A'$.
由于每个正有理数都是某条与 $E$ 可通约的线段的长度, 而 $\Sigma$ 本身与 $E$ 不可通约,
所以每个有理数恰好归入上述两组之一, 从而得到有理数域中的分划 $A|A'$.
